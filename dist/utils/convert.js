...
export const convertPresence = async (presence) => {
    ...
    let result = '';
    platform.forEach((pf) => {
        if (!presence.hasOwnProperty(pf)) {
            result += `${pf}: <:status_offline:908249115505332234>\n`;
        }
        else {
            switch (presence[pf]) {
                case 'online':
...
export const convertFlags = async (bitField) => {
    ...
    if (bitField == 0)
        return 'None';
    if (bitField & (0b1 << 0))
        flags.push('<:discord_staff:908516650331025458>');
    if (bitField & (0b1 << 1))
        flags.push('<:discord_partner:908517486113202287>');
    if (bitField & (0b1 << 2))
        flags.push('<:hypeSquad_event:908246675108266024>');
    if (bitField & (0b1 << 3))
        flags.push('<:bug_hunter:908516650133897287>');
    if (bitField & (0b1 << 6))
        flags.push('<:Hypesquad_bravery:908246674818871306>');
    if (bitField & (0b1 << 7))
        flags.push('<:hypesquad_brilliance:908246675116670986>');
    if (bitField & (0b1 << 8))
        flags.push('<:hypesquad_balance:908246674755964978>');
    if (bitField & (0b1 << 9))
        flags.push('<:early_supporter:908246674743365632>');
    if (bitField & (0b1 << 10))
        flags.push('TEAM_PSEUDO_USER');
    if (bitField & (0b1 << 14))
        flags.push('<:bug_buster:908516650326843412>');
    if (bitField & (0b1 << 16))
        flags.push('<:verified_bot:936164801674100746>');
    if (bitField & (0b1 << 17))
        flags.push('<:verified_developer:908246675267657759>');
    if (bitField & (0b1 << 18))
        flags.push('<:discord_certified_moderator:908246674516885544>');
    if (bitField & (0b1 << 19))
        flags.push('BOT_HTTP_INTERACTIONS');
    return flags.join(' ');
};
export const convertPermissions = async (bitField) => {
    ...
    if (bitField & (0x1 << 31))
        perms.push('use_application_commands');
    if (bitField & (0x1 << 32))
    ...
    if (bitField & (0x1 << 39))
        perms.push('start_embedded_activities');
    if (bitField & (0x1 << 40))
