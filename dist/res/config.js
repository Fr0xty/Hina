...
Hina.player.on('trackStart', (queue, track) => {
    const embed = new MessageEmbed()
        ...
        .addFields({ name: 'Source', value: track.source }, { name: 'Artist', value: track.author }, { name: 'Views', value: String(track.views) }, { name: 'Requested by', value: `<@${track.requestedBy.id}>` });
...
Hina.player.on('trackAdd', async (queue, track) => {
    const embed = new MessageEmbed()
        .setColor(Hina.color)
        .setAuthor({
        name: `Music queue for ${queue.guild.name}`,
...
Hina.player.on('tracksAdd', async (queue, track) => {
    const embed = new MessageEmbed()
        .setColor(Hina.color)
        .setAuthor({
        name: `Music queue for ${queue.guild.name}`,
...
Hina.player.on('queueEnd', async (queue) => {
