    ...
    constructor() {
        ...
        this.args = [
            new CommandArgument({ optional: true })
                .setName('to_language')
                .setDescription('Translate to what language? Leaving blank will default to english. Only accept `ISO 639-1` & `ISO 639-2` language code.'),
            new CommandArgument({ optional: true })
                .setName('from_language')
                .setDescription('Translate from what language? Leaving blank will auto detect language. Only accept `ISO 639-1` & `ISO 639-2` language code.'),
        ];
    }
    async execute(msg, args) {
        let [to_language, from_language] = args;
        if (!msg.reference)
            return await msg.reply('Please reply to the message you want to translate.');
