...
export default class jisho {
    ...
    async execute(msg, args) {
        ...
        if (result.meta.status !== 200)
            return await msg.reply(`No result was found with the search term \`${word}\`.`);
        let pages = [];
        for (let i = 0; i < result.data.length; i++) {
            ...
            if (wordResult.tags.length)
                tagList.push(wordResult.tags[0]);
            ...
            if (wordResult.japanese.length) {
                wordResult.japanese.forEach((alt) => {
                    if (alt.word && alt.reading)
                        alternativeKanji += `-${alt.word} (${alt.reading})\n`;
                    else if (alt.word)
                        alternativeKanji += `-${alt.word}\n`;
                    else
                        alternativeKanji += `-${alt.reading}\n`;
                });
            }
            else {
                alternativeKanji = '';
