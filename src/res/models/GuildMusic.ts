...
import { MessageEmbed, NewsChannel, TextChannel, VoiceBasedChannel } from 'discord.js';

import { Hina, hinaColor } from '../config.js';
import { guildOrHinaIcon } from '../../utils/general.js';
import { convertSeconds } from '../../utils/convert.js';

export default class GuildMusic {
    vc: VoiceBasedChannel;
    mc: TextChannel | NewsChannel;
    songs: any[];
    loop: Boolean;
    player: AudioPlayer;
    subscription: PlayerSubscription;

    constructor(vc: VoiceBasedChannel, mc: TextChannel | NewsChannel) {
        this.vc = vc;
        this.mc = mc;
        this.songs = [];
        this.loop = false;
        this.player = createAudioPlayer();
        this.subscription = getVoiceConnection(vc.guildId)!.subscribe(this.player)!;
    }

    async _playStart() {
        this.player.play(this.songs[0].resource);
    ...
    async _nowPlayingAnnounce() {
        const authorIcon = await guildOrHinaIcon(Hina, this.vc.guild);
        ...
        const embed = new MessageEmbed()
            ...
            .addFields(
                ...
                {
                    name: 'Channel Owner',
                    value: `[${npSongInfo.videoDetails.author.name}](${npSongInfo.videoDetails.author.channel_url})`,
                    inline: true,
                },
    ...
    async _songAddAnnounce() {
        ...
        const embed = new MessageEmbed()
            ...
            .setDescription(
                `[${this.songs.at(-1).videoInfo.videoDetails.title}](${
                    this.songs.at(-1).videoInfo.videoDetails.video_url
                })`
            )
    ...
    async updateChannels(vc: VoiceBasedChannel | null, mc: TextChannel | NewsChannel | null) {
        if (vc) this.vc = vc;
        if (mc) this.mc = mc;
    }

    async addSong(resource: AudioResource, videoInfo: any) {
        let firstTime = false;
    ...
    async getQueue() {
        ...
        let page = `**Now Playing:**\n[${this.songs[0].videoInfo.videoDetails.title}](${
            this.songs[0].videoInfo.videoDetails.video_url
        }) \`${await convertSeconds(this.songs[0].videoInfo.videoDetails.lengthSeconds)}\`\n\n`;
        let pages = [];
